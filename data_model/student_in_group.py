...
class StudentInGroup(AbstractModel):
    ...
    @staticmethod
    def parse(file_location, db_source: FileSource) -> List[(Optional[str], Optional[StudentInGroup])]:
        ...
        for i in lines:
            try:
                student_id = int(i[0])
                group_id = int(i[1])
                res.append(ParsedData(None, StudentInGroup(db_source, student_id, group_id)))

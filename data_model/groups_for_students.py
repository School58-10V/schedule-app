...
class GroupsForStudents(AbstractModel):
    """
        Класс группы от ученика. Используется для m2m отношения между
        Group и Student
    ...
    @staticmethod
    def parse(file_location) -> List[(Optional[str], Optional[GroupsForStudents])]:
        ...
        for i in lines:
            try:
                student_id = int(i[0])
                group_id = int(i[1])
                res.append(ParsedData(None, GroupsForStudents(student_id, group_id)))
    ...
    def _get_collection_name(cls):
        # переопределяем метод, который отвечает за имя класса,
        # чтобы отдавал другое и лез в ту базу, которую и надо

...
class LessonRowTransformationService:
    ...
    def get_all_lesson_rows_transform(self, db_source: DBSource):
        global_dct = {'lesson_rows': []}
        for i in LessonRow.get_all(db_source):
            local_dct = i.__dict__()
            local_dct['teachers'] = [i.get_main_id() for i in TeachersForLessonRows.
                get_teachers_by_lesson_row_id(i.get_main_id(), db_source=db_source)]
            global_dct['lesson_rows'].append(local_dct.copy())
        return global_dct

    def get_all_detailed_transform(self, db_source: DBSource):
        global_dct = {'lesson_rows': []}
        for i in LessonRow.get_all(db_source):
            local_dct = i.__dict__()
    ...
    def get_lesson_row_by_id_transform(self, object_id: int, db_source: DBSource):
        dct = LessonRow.get_by_id(object_id, db_source).__dict__()
        dct['teachers'] = [i.get_main_id() for i in TeachersForLessonRows.
            get_teachers_by_lesson_row_id(object_id, db_source=db_source)]
        return dct

    def get_detailed_lesson_row_by_id_transform(self, object_id: int, db_source: DBSource):
        dct = LessonRow.get_by_id(object_id, db_source).__dict__()
        dct['teachers'] = [i.__dict__() for i in TeachersForLessonRows.
            get_teachers_by_lesson_row_id(object_id, db_source=db_source)]
        return dct

    def create_lesson_row_transform(self, request: dict, db_source: DBSource):
        dct = request
    ...
    def update_lesson_rows_transform(self, object_id: int, request: dict, db_source: DBSource):
        dct = request
        new_teachers_id = dct.pop("teachers")
        lesson_row_by_id = LessonRow.get_by_id(object_id, db_source)
    ...
    def delete_lesson_row_transform(self, object_id: int, db_source: DBSource):
        lesson_row = LessonRow.get_by_id(object_id, db_source)
        lesson_row = lesson_row.delete().__dict__()
        return lesson_row

    def check_availability(self, object_id: int, db_source: DBSource):
        LessonRow.get_by_id(object_id, db_source=db_source)

...
class DBSource(AbstractSource):
    ...
    def connect(self, retry_count: int = 3):
        if self.__conn:
            if self.__timestamp:
                if (self.get_actual_timestamp() - self.__timestamp).total_seconds() >= 300:
                    try:
                        self.__conn.close()
                    finally:
                        self.__conn = None
                        self.__timestamp = None
                        self.connect()
            self.__timestamp = self.get_actual_timestamp()
            return
    ...
    @classmethod
    def __cursor_execute_wrapper(cls, cursor: any, request: str, params=None):  # TODO: обрабатывать так все методы
        try:
            cursor.execute(request, params)
        except psycopg2.OperationalError as e:
            raise ConnectionError('Сервер незапланированно отключил соединение до или во время выполнения задачи.')
        except psycopg2.Error as e:

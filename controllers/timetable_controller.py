...
def create_timetable():
    return jsonify(TimeTable(**request.get_json(), db_source=app.config.get("schedule_db_source")) \
        .save() \
        .__dict__()
                   )
...
def update_timetable(object_id):
    try:
        ...
        return "", 404
    return jsonify(TimeTable(**request.get_json(), object_id=object_id, db_source=app.config.get("schedule_db_source")) \
        .save() \
        .__dict__()
                   )
...
def delete_timetable(object_id):
    try:
        return jsonify(TimeTable.get_by_id(object_id, db_source=app.config.get("schedule_db_source")).delete().__dict__())
    except ValueError:
        return "", 404

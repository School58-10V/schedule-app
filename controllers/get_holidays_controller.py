...
def get_holidays_by_year(year: int):
    ...
    login = data['login']
    try:
        name = User.get_by_login(login, source=app.config.get('auth_db_source')).get_name()
    except ValueError:
        return '', 401
    ...
    teacher = Teacher.get_by_name(name=name, source=app.config.get('schedule_db_source'))
    if len(student + teacher) == 0:
        return '', 401

    try:
        timetable_id = TimeTable.get_by_year(year=year, source=app.config.get('schedule_db_source')).get_main_id()

...
from flask import Flask, request, jsonify, app, Response
...
def create_lesson():
    return jsonify(Lesson(**request.get_json(), db_source=dbf.get_db_source()).save().__dict__())
...
def update_lessons(object_id: int):
    try:
        ...
        return "", 404
    return Lesson(**request.get_json(), object_id=object_id, db_source=dbf.get_db_source()).save().__dict__()

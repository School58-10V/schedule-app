...
def get_teacher_for_lesson_rows() -> Response:
    return jsonify([i.__dict__() for i in TeachersForLessonRows.get_all(app.config.get("schedule_db_source"))])


@app.route("/api/v1/teacher_for_lesson_rows/<int:object_id>", methods=["GET"])
def get_teacher_for_lesson_rows_by_id(object_id: int) -> Union[Response, Tuple[str, int]]:
    try:
